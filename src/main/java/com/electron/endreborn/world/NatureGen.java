...
public class NatureGen {
	public static void initGen() {
		...
		if(ModConfigs.COMMON.balance.end_decorator.get()) {
			...
			Biomes.SMALL_END_ISLANDS.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new XorciteClusterFeature(NoFeatureConfig.field_236558_a_).withConfiguration(IFeatureConfig.NO_FEATURE_CONFIG).withPlacement(Placement.COUNT_HEIGHTMAP_DOUBLE.configure(new FrequencyConfig(ModConfigs.COMMON.balance.xorcite_clusters_rarity.get()))));
