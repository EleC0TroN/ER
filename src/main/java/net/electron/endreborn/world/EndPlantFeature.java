...
public class EndPlantFeature extends Feature<OreFeatureConfig> {
	public EndPlantFeature(Function<Dynamic<?>, ? extends OreFeatureConfig> config) {
        super(config);
	}
		                        ...
		                        for(int j2 = l; j2 <= k1; ++j2) {
		                           ...
		                           if (d8 * d8 + d9 * d9 + d10 * d10 < 2.0D) {
		                              ...
		                              if (!bitset.get(k2)) {
		                                 ...
		                                 if (world.getBlockState(blockpos$mutable.down()).getBlock() == net.electron.endreborn.blocks.Blocks.END_MOSS && config.state.canPlaceAt(world, blockpos$mutable) && world.isAir(blockpos$mutable))
		                                 {
